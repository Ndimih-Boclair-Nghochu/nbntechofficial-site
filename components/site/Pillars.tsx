...
/**
 * Expertise as numbered service cards (hooyia "What we offer" pattern): a large
 * ghosted index, an icon tile, and the capability with proof points.
 */
    ...
    <RevealGroup className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
      {pillars.map((p, i) => (
        <RevealItem
          key={p.key}
          as="article"
          className="group relative flex flex-col overflow-hidden rounded-xl2 border border-ink-line bg-white p-7 shadow-card transition-all duration-300 hover:-translate-y-1.5 hover:border-cyan/40 hover:shadow-card-hover"
        >
          <span
            aria-hidden
