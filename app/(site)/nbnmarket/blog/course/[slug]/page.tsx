  ...
  const faqs = [
    {
      ...
      a: `If you want to ${catName.toLowerCase().includes("course") ? "build these skills" : `learn ${catName.toLowerCase()}`}, it's a solid pick${c.rating ? ` — it currently holds a ${c.rating}★ rating${c.reviewCount ? ` from ${Number(c.reviewCount).toLocaleString("en-US")} reviews` : ""}` : ""}. Check the live price and enrol if it fits your goals and budget.`,
    },
    ...
    <>
      ...
      <Container className="pb-6">
        ...
        <article className="mx-auto max-w-3xl">
          <span className="text-xs font-bold uppercase tracking-wide text-[#c77b00]">🎓 {catName} · Course review · {currentYear()}</span>
          <h1 className="mt-2 font-serif text-3xl font-bold text-ink sm:text-4xl">{title}</h1>

          {c.image && (
            /* eslint-disable-next-line @next/next/no-img-element */
            <img src={c.image} alt={c.imageAlt || c.title} loading="lazy" className="mt-6 w-full rounded-xl border border-ink-line bg-white object-cover" />
          )}

          <p className="mt-6 text-lg leading-relaxed text-ink-body">
            {c.shortDescription || c.description || `${c.title} is an online course on ${c.provider}.`}{" "}
            Here’s what you’ll learn, who it’s for, and whether it’s worth enrolling in {currentYear()}. Course prices
            on {c.provider} change often — we link you straight to the live price.
          </p>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3 rounded-xl bg-sand-soft p-4">
            ...
            <div className="ml-auto flex flex-wrap gap-2">
              ...
              {url && (
                <a href={url} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center rounded-lg border border-ink-line px-4 py-2 text-sm font-semibold text-ink hover:border-cyan hover:text-cyan-deep">
              ...
              <ul className="mt-4 grid gap-2.5">
                {c.whatYouLearn.slice(0, 10).map((x, i) => (
                  <li key={i} className="flex gap-2.5">
                    ...
                    <span className="text-ink-body">{x}</span>
                  </li>
                ))}
              </ul>
            </section>
          ...
          <section className="mt-10">
            ...
            <dl className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
              {details.map((d) => (
                <div key={d.label} className="rounded-lg border border-ink-line bg-surface p-3">
          ...
          {c.requirements.length > 0 && (
            <section className="mt-10">
              <h2 className="font-serif text-2xl font-bold text-ink">Requirements</h2>
              <ul className="mt-3 list-disc space-y-1.5 pl-5 text-ink-body">
                {c.requirements.slice(0, 8).map((x, i) => <li key={i}>{x}</li>)}
              </ul>
            </section>
          )}
          ...
          <p className="mt-8">
            <Link href="/courses" className="font-medium text-cyan-deep hover:underline">→ Browse more online courses</Link>
          </p>
        </article>
